...
class Schedule:
    ...
    def check_deadline(self, deadline, estimated_time):
        ...
        start_date = datetime.datetime(int(start_date[0]), int(start_date[1]), int(start_date[2]), int(start_time.split(":")[0]), int(start_time.split(":")[1]))

        deadline = deadline.split()
        deadline_y = deadline[0].split(".")
        deadline_t = deadline[1].split(":")
        deadline = datetime.datetime(int(deadline_y[0]), int(deadline_y[1]), int(deadline_y[2]), int(deadline_t[0]), int(deadline_t[1]))
        ...
        for i in range(len(self.json_str["schedule"])):
            ...
            #sch_dic에 저장
                ...
                if schedule["deadline"] != None:
                    deadline_check = self.check_deadline(schedule["deadline"], run_time)
                    if deadline_check[0] == 1:
                        (row, column) = self.make_deadline(schedule["deadline"])
                        self.sch_dic[self.count] = (schedule["name"], deadline_check[1], (row, column))
                    else:
                        self.sch_dic[self.count] = (schedule["name"], deadline_check[1], None)
                else:
                        ...
                        if schedule[j][k] == i+2:
                            ...
                            if idx >= int(60/self.time_unit):
                                ...
                                if idx > int(60/self.time_unit) * 3:
                                    fitness -= 1
                            elif idx < int((60/self.time_unit)/2):
                                    fitness -= 2
                            k += idx
                            
                        #deadline전 deadline_count가 run_time보다 작을 때 0이 존재하는 경우
                        elif schedule[j][k] == 0:
                            idx_ = 1
                            while True:
                                if k+idx_ < int(24*60/self.time_unit) and schedule[j][k+idx_] == 0:
                                    idx_ += 1
                                else:
                                    break
                            if (j < dic_[2][0] or (j == dic_[2][0] and k <= dic_[2][1])):
                                if total_count < run_time and idx_ >= 2:
                                    fitness -= idx_/len(self.sch_dic)
                            k += idx_
                ...
                for j in range(self.week_len):
                    ...
                    while k < int(24*60/self.time_unit):
                        if schedule[j][k] == i+2:
                            idx = 1
                            while True:
                                if k+idx < int(24*60/self.time_unit) and schedule[j][k+idx] == i+2:
                                    idx += 1
                                else:
                                    break
                            total_count += idx
                            ...
                            if (j < dic_[2][0] or (j == dic_[2][0] and k <= dic_[2][1])):
                                ...
                                if idx >= int(60/self.time_unit):
                                    ...
                                    if idx > int(60/self.time_unit) * 3:
                                        fitness -= 1
                                elif idx < int(60/self.time_unit) and deadline_count < run_time:
                                    fitness -= 2
    ...
    def select_survivors(self, population_sorted, best_sample, lucky_few):
        ...
        count = 0
        while count < best_sample:
            next_generation.append(population_sorted[0][0])
            population_sorted.pop()
            count += 1
    ...
    def execute_algorithm(self):
        ...
        mutation_insert = 8
        #원하지 않는 방향으로 수렴하는 걸 막기 위해 매 400번 반복마다 generate_population을 수행할 개체수
        convergence_esc = 48
...
#             elif count % 100 == 0:
#                 print(count, "th : ", time.time() - start_time, "sec")
...
#                 print("SUCCESS! The plan is============================================= \n", (pop_sorted[0]))
            ...
            if count % 300 == 0:
                pop = self.generate_population(convergence_esc)
                next_generation = self.select_survivors(pop_sorted, 16, 0)
                pop_sorted = self.compute_performance(pop)
                for i in range(convergence_esc):
                    next_generation.append(pop_sorted[i][0])
                random.shuffle(next_generation)
            else:
