...
import com.capstone.AreyouP.job.JobRepository;
import com.capstone.AreyouP.job.seperated.SeperatedJob;
import com.capstone.AreyouP.job.seperated.SeperatedJobRepository;
...
//                                bigJob = j.get();
    ...
    public List<TimeLineDto> getTable(String startDate, String endDate, Long user_id){
        Date start = new Date();
        Date end = new Date();
        try {
        ...
        for (Map.Entry<String, List<JobDto>> entry : getJobsOfDay.entrySet()){
            ...
            timeLineDto.setDay(entry.getKey());
            timeLineDto.setSubject(entry.getValue());
