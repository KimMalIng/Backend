...
public interface CustomizeJobRepository extends JpaRepository<CustomizeJob, Long> {
    ...
    @Query("SELECT cj FROM CustomizeJob cj WHERE cj.startDate BETWEEN :start AND :end AND cj.isFixed = true And cj.member.id = :id")
    ...
    @Query("SELECT cj FROM CustomizeJob cj WHERE cj.startTime IS NOT NULL " +
            ...
            "AND cj.isFixed = true AND cj.member.id = :id")
