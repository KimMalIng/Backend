...
public interface SeperatedJobRepository extends JpaRepository<SeperatedJob, Long> {
    ...
    @Transactional
    void deleteAllByDayIsGreaterThanEqualAndNameAndMemberIdAndIsCompleteIsFalse(LocalDate day, String name, Long memberId);

    @Transactional
    void deleteAllByDayAfterAndNameAndIsCompleteIsFalse(LocalDate now, String name);

    List<SeperatedJob> findAllByMemberId(Long memberId);

    List<SeperatedJob> findAllByNameAndMemberId(String name, Long aLong);
}
